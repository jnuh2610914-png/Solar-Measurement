  ...
  useEffect(() => {
    ...
    const loadAndRenderMap = async () => {
      try {
        ...
        // 💡 1. 여기에 직접 발급받으신 카카오 JavaScript Key를 넣으실 수 있습니다!
        ...
        const initializeMap = () => {
          // 💡 3. 지도 영역을 그리는 div의 id("kakao-map")와 100% 일치합니다.
          ...
          (window as any).kakao.maps.load(() => {
            ...
            // ... (중략: 기존의 고정밀 지도 제어, 마커 및 드래그, 리버스 지오코더 로직 완벽 유지) ...
          });
        };
        ...
        if ((window as any).kakao && (window as any).kakao.maps && (window as any).kakao.maps.services) {
          ...
          if (script) {
            script.remove(); // Force reload if necessary
          }
          
          script = document.createElement("script");
          script.id = scriptId;
          // 💡 2. 'https:' 프로토콜을 명시하여 완벽한 경로로 주입합니다.
          script.src = `https://dapi.kakao.com/v2/maps/sdk.js?appkey=••••••••••••••••••••••••••••••••&autoload=false&libraries=services`;
          script.onload = () => {
            initializeMap();
          };
